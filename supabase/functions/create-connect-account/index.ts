...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!storeId || !email) throw new UserFacingError('storeId y email son obligatorios');
    ...
    if (!store) throw new UserFacingError('Tienda no encontrada');
    ...
    console.error('[create-connect-account]', err instanceof Error ? err.message : String(err));
