...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!store.stripe_connect_account_id) {
      throw new UserFacingError('Esta tienda no tiene cuenta Stripe Connect configurada');
    }
    ...
    console.error('[create-login-link]', err instanceof Error ? err.message : String(err));
